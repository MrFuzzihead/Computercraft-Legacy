...
| 10 | Fix ~~`Alarm.compareTo`~~ **DONE (see B2)**, `TurtleBrain.setOverlay`, `FileMount.list` NPE, `JarMount.m_root` NPE, `getFreeSpace` sync, `TermAPI` getter locking, dead `m_busy`/`wrapBufferedReader` | 🟡       | Small  | various                          |
