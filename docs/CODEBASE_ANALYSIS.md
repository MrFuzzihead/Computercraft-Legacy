...
| 9  | ~~`fs.open` error messages for all modes~~ **DONE (see B4)** — uniform `nil, message` + null-handle guards + `FSAPITest` (9 tests) and `IOAPITest` (1 test); `term.blit` colour validation                                                                                                           | 🟡       | Small  | `core/apis`                      |
