...
| 10 | Fix ~~`Alarm.compareTo`~~ **DONE (see B2)**, ~~`TurtleBrain.setOverlay`~~ **DONE (see B3)**, `FileMount.list` NPE, `JarMount.m_root` NPE, ~~`getFreeSpace` sync~~ **DONE (see B8)**, ~~`TermAPI` getter locking~~ **DONE (see B9)**, ~~dead `m_busy`~~ **DONE (removed in P1)**/`wrapBufferedReader` | 🟡       | Small  | various                          |
