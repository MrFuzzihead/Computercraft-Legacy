...
6. 🟠 ~~`ComputerThread` concurrency hazards (unsynchronized `WeakHashMap`, silently dropped tasks)~~ **BOTH FIXED** (see C1 and C2); the serialized thread-per-task model (P1) remains.
