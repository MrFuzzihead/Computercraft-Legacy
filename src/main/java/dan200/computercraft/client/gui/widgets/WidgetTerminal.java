...
public class WidgetTerminal extends Widget {
    ...
    public WidgetTerminal(int x, int y, int termWidth, int termHeight, IComputerContainer computer, int leftMargin,
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        if (mouseX >= this.getXPosition() && mouseX < this.getXPosition() + this.getWidth()
            ...
            if (this.m_focus) {
                ...
                if (!this.m_locked && computer != null && computer.isColour() && button >= 0 && button <= 2) {
                    ...
                    if (term != null) {
                        int charX = (mouseX - (this.getXPosition() + this.m_leftMargin))
                            / FixedWidthFontRenderer.FONT_WIDTH;
                        int charY = (mouseY - (this.getYPosition() + this.m_topMargin))
                            / FixedWidthFontRenderer.FONT_HEIGHT;
        ...
        if (mouseX >= this.getXPosition() && mouseX < this.getXPosition() + this.getWidth()
            ...
            if (term != null) {
                ...
                if (this.m_lastClickButton >= 0 && !Mouse.isButtonDown(this.m_lastClickButton)) {
                    if (this.m_focus && !this.m_locked) {
                        computer
                            .queueEvent("mouse_up", new Object[] { this.m_lastClickButton + 1, charX + 1, charY + 1 });
                    }
                ...
                if (this.m_focus && !this.m_locked) {
                    ...
                    if (this.m_lastClickButton >= 0 && (charX != this.m_lastClickX || charY != this.m_lastClickY)) {
                        computer.queueEvent(
                            "mouse_drag",
                            new Object[] { this.m_lastClickButton + 1, charX + 1, charY + 1 });
                        this.m_lastClickX = charX;
                        this.m_lastClickY = charY;
                    }
                }
