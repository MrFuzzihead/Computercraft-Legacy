...
public class TileDiskDrive extends TilePeripheralBase implements IInventory {
   ...
   public boolean onActivate(EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
      if (player.isSneaking()) {
         if (!this.getWorldObj().isRemote) {
            ItemStack disk = player.func_71045_bC();
   ...
   public String getName() {
      return this.hasCustomName() ? this.getLabel() : "tile.computercraft:drive.name";
   }
      ...
      if (this.m_diskStack != null) {
         ...
         if (contents != null) {
            if (this.m_diskMount == null) {
               this.m_diskMount = contents.createDataMount(this.m_diskStack, this.getWorldObj);
            }
   ...
   private synchronized void ejectContents(boolean destroyed) {
      if (!this.getWorldObj().isRemote) {
         if (this.m_diskStack != null) {
            ...
            EntityItem entityitem = new EntityItem(this.getWorldObj(), x, y, z, disks);
            entityitem.motionX = xOff * 0.15;
            entityitem.motionY = 0.0;
            entityitem.motionZ = zOff * 0.15;
            this.getWorldObj().spawnEntityInWorld(entityitem);
            if (!destroyed) {
               this.getWorldObj().func_72926_e(1000, this.xCoord, this.yCoord, this.zCoord, 0);
            }
         }
      }
   }
   ...
   private void playRecord() {
      ...
      if (record != null) {
         ComputerCraft.playRecord(
            record, contents.getAudioTitle(this.m_diskStack), this.getWorldObj(), this.xCoord, this.yCoord, this.zCoord
         );
      } else {
         ComputerCraft.playRecord(null, null, this.getWorldObj(), this.xCoord, this.yCoord, this.zCoord);
      }
   }
