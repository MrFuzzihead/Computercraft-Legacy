...
public class TilePrinter extends TilePeripheralBase implements IInventory, ISidedInventory {
    ...
    public ItemStack decrStackSize(int i, int j) {
        synchronized (this.m_inventory) {
            if (this.m_inventory[i] == null) {
                return null;
            } else if (this.m_inventory[i].stackSize <= j) {
    ...
    private boolean isPaper(ItemStack stack) {
        ...
        return item == Items.paper
            || item instanceof ItemPrintout && ItemPrintout.getType(stack) == ItemPrintout.Type.Single;
    }
        ...
        synchronized (this.m_inventory) {
            for (int i = 0; i < 13; i++) {
                ...
                if (stack != null) {
                    ...
                    EntityItem entityitem = new EntityItem(this.worldObj, x, y, z, stack);
                    entityitem.motionX = this.worldObj.rand.nextFloat() * 0.2 - 0.1;
                    entityitem.motionY = this.worldObj.rand.nextFloat() * 0.2 - 0.1;
                    entityitem.motionZ = this.worldObj.rand.nextFloat() * 0.2 - 0.1;
