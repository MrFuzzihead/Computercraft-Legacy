...
public class ItemCable extends ItemPeripheralBase {
   ...
   public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float fx, float fy, float fz) {
      if (!this.func_150936_a(world, x, y, z, side, player, stack)) {
            ...
            if (existingType == PeripheralType.WiredModem && type == PeripheralType.Cable) {
               if (stack.stackSize > 0) {
                  ...
                  world.playSoundEffect(
                     ...
                     y + 0.5,
                     z + 0.5,
                     ComputerCraft.Blocks.cable.field_149762_H.func_150495_a(),
                     (ComputerCraft.Blocks.cable.field_149762_H.func_150497_c() + 1.0F) / 2.0F,
                     ComputerCraft.Blocks.cable.field_149762_H.func_150494_d() * 0.8F
                  );
                  stack.stackSize--;
         ...
         if (existing != Blocks.field_150350_a && (type == PeripheralType.Cable || existing.isSideSolid(world, x, y, z, ForgeDirection.getOrientation(side)))) {
