...
public class TileMonitor extends TilePeripheralBase implements ITerminalTile {
   ...
   public IIcon getTexture(int side) {
      IIcon[] texArray = this.getLocalTerminal().isColour() ? s_advancedIcons : s_icons;
   ...
   public final void readDescription(NBTTagCompound nbttagcompound) {
      ...
      this.m_changed = true;
      if (oldXIndex != this.m_xIndex
         || oldYIndex != this.m_yIndex
   ...
   private void rebuildTerminal() {
      ...
      int termWidth = Math.max(Math.round((this.m_width - 0.3125F) / (textScale * 6.0F * RENDER_PIXEL_SCALE)), 1);
      int termHeight = Math.max(Math.round((this.m_height - 0.3125F) / (textScale * 9.0F * RENDER_PIXEL_SCALE)), 1);
   ...
   private TileMonitor getSimilarMonitorAt(int x, int y, int z) {
      if (y >= 0 && y < this.getWorldObj().getHeight() && this.getWorldObj().blockExists(x, y, z)) {
         TileEntity tile = this.getWorldObj().getTileEntity(x, y, z);
         if (tile != null && tile instanceof TileMonitor) {
            TileMonitor monitor = (TileMonitor)tile;
            if (monitor.getDir() == this.getDir()
               && monitor.getLocalTerminal().isColour() == this.getLocalTerminal().isColour()
               && !monitor.m_destroyed
               && !monitor.m_ignoreMe) {
               return monitor;
            }
      ...
      for (int y = 0; y < height; y++) {
         for (int x = 0; x < width; x++) {
            TileMonitor monitor = this.getSimilarMonitorAt(
               this.xCoord + rightX * x + downX * y, this.yCoord + rightY * x + downY * y, this.zCoord + rightZ * x + downZ * y
            );
            if (monitor != null) {
               monitor.m_xIndex = x;
               monitor.m_yIndex = y;
      ...
      if (!(pair.x > this.m_width - RENDER_BORDER) && !(pair.y > this.m_height - RENDER_BORDER) && !(pair.x < RENDER_BORDER) && !(pair.y < RENDER_BORDER)) {
         ...
         if (originTerminal != null) {
            if (this.getTerminal().isColour()) {
               ...
               int xCharPos = (int)Math.min((float)originTerminal.getWidth(), Math.max((pair.x - RENDER_BORDER - RENDER_MARGIN) / xCharWidth + 1.0F, 1.0F));
               int yCharPos = (int)Math.min((float)originTerminal.getHeight(), Math.max((pair.y - RENDER_BORDER - RENDER_MARGIN) / yCharHeight + 1.0F, 1.0F));
   ...
   public AxisAlignedBB getRenderBoundingBox() {
      if (this.getXIndex() == 0 && this.getYIndex() == 0) {
         ...
         if (monitor != null) {
            ...
            return ComputerCraft.Blocks.peripheral.getSelectedBoundingBoxFromPool(this.getWorldObj(), this.xCoord, this.yCoord, this.zCoord);
         }
      } else {
         return ComputerCraft.Blocks.peripheral.getSelectedBoundingBoxFromPool(this.getWorldObj(), this.xCoord, this.yCoord, this.zCoord);
      }
