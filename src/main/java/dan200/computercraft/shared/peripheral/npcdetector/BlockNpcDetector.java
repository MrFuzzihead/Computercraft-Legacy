...
public class BlockNpcDetector extends BlockGeneric {
    ...
    public static void registerIcons(IIconRegister iconRegister) {
        s_icons[0] = iconRegister.registerIcon("computercraft:npcDetectorTop");
        s_icons[1] = iconRegister.registerIcon("computercraft:npcDetectorSide");
        s_icons[2] = iconRegister.registerIcon("computercraft:npcDetectorFront");
    }

    public static IIcon getNpcDetectorIcon(int side, int direction) {
        if (side == 0 || side == 1) return s_icons[0]; // top / bottom
        if (side == direction) return s_icons[2]; // front face
        return s_icons[1]; // side faces
    }

    public BlockNpcDetector() {
        super(Material.iron);
        this.setHardness(2.0F);
        this.setBlockName("computercraft:npc_detector");
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entity, ItemStack stack) {
        ...
        if (tile instanceof TileNpcDetector) {
            ((TileNpcDetector) tile).setDirection(dir);
        }
    }
}
