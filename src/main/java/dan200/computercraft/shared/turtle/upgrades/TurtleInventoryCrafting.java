    ...
    private ItemStack tryCrafting(int xOffset, int yOffset) {
        ...
        for (int i = 0; i < 4; i++) {
            if (this.m_turtle.getInventory()
                .getStackInSlot(emptyColumn + i * 4) != null
                || this.m_turtle.getInventory()
                    .getStackInSlot(i + emptyRow * 4) != null) {
                return null;
            }
        }
        ...
        return CraftingManager.getInstance()
            .findMatchingRecipe(this, this.m_turtle.getWorld());
    }
            ...
            if (result != null) {
                ...
                for (int nx = 0; nx < size; nx++) {
                    ...
                    if (stack != null) {
                        ...
                        ItemStack replacement = stack.getItem()
                            .getContainerItem(stack);
                        if (replacement != null && this.getStackInSlot(nx) == null
                            && (!replacement.isItemStackDamageable()
                                || replacement.getItemDamage() < replacement.getMaxDamage())) {
