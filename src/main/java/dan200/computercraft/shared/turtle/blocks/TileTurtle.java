...
public class TileTurtle extends TileComputerBase implements ITurtleTile, IInventory {
   ...
   protected final ServerComputer createComputer(int instanceID, int id, int termWidth, int termHeight) {
      ServerComputer computer = new ServerComputer(this.getWorldObj(), id, this.m_label, instanceID, this.getFamily(), termWidth, termHeight);
      ...
      if (!this.hasMoved()) {
         ...
         if (!this.getWorldObj().isRemote) {
            ...
            for (int i = 0; i < size; i++) {
               ...
               if (stack != null) {
                  WorldUtil.dropItemStack(stack, this.getWorldObj(), this.xCoord, this.yCoord, this.zCoord);
               }
            }
         }
      } else {
   ...
   public boolean onActivate(EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
      ...
      if (currentItem != null) {
         ...
         if (currentItem.getItem() == Items.field_151131_as && this.m_brain.getDyeColour() != -1) {
            if (!this.getWorldObj().isRemote && this.m_brain.getDyeColour() != -1) {
               this.m_brain.setDyeColour(-1);
               if (!player.capabilities.isCreativeMode) {
                  currentItem.func_150996_a(Items.field_151133_ar);
               }
            }
   ...
   public void func_145845_h() {
      ...
      synchronized (this.m_inventory) {
         if (!this.getWorldObj().isRemote && this.m_inventoryChanged) {
            IComputer computer = this.getComputer();
            if (computer != null) {
