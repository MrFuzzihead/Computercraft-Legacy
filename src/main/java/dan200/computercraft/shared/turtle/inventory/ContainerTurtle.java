...
public class ContainerTurtle extends Container {
   ...
   protected ContainerTurtle(IInventory playerInventory, ITurtleAccess turtle, int playerInvStartY, int turtleInvStartX) {
      ...
      for (int y = 0; y < 4; y++) {
         for (int x = 0; x < 4; x++) {
            this.addSlotToContainer(new Slot(this.m_turtle.getInventory(), x + y * 4, turtleInvStartX + 1 + x * 18, playerInvStartY + 1 + y * 18));
         }
      }

      for (int y = 0; y < 3; y++) {
         for (int x = 0; x < 9; x++) {
            this.addSlotToContainer(new Slot(playerInventory, x + y * 9 + 9, 8 + x * 18, playerInvStartY + 1 + y * 18));
         }
      }
   ...
   public boolean canInteractWith(EntityPlayer player) {
      ...
      return turtle != null ? turtle.isUsableByPlayer(player) : false;
   }
