...
public class ItemPocketComputer extends Item implements IComputerItem, IMedia {
     ...
     * ({@code upgrade=2}).
     */
    public ItemStack createWithEnderModem(int id, String label, ComputerFamily family) {
        if (family != ComputerFamily.Normal && family != ComputerFamily.Advanced) {
            return null;
        }
        int damage = family == ComputerFamily.Advanced ? 1 : 0;
    ...
    /**
     * Creates a pocket-computer item stack with a Chat Box installed
     * ({@code upgrade=4}).
     */
    public ItemStack createWithChatBox(int id, String label, ComputerFamily family) {
        if (family != ComputerFamily.Normal && family != ComputerFamily.Advanced) {
            return null;
        }
        int damage = family == ComputerFamily.Advanced ? 1 : 0;
    ...
    public void getSubItems(Item itemID, CreativeTabs tabs, List list) {
        ...
        list.add(PocketComputerItemFactory.createWithChatBox(-1, null, ComputerFamily.Normal));
        ...
        if (!world.isRemote) {
            ...
            if (computer != null) {
                ...
                if (peripheral != null && peripheral instanceof PocketModemPeripheral) {
                    ...
                    speaker.tick();
                } else if (peripheral instanceof PocketChatBoxPeripheral) {
                    PocketChatBoxPeripheral chatBox = (PocketChatBoxPeripheral) peripheral;
                    int dimId = world.provider.dimensionId;
                    if (entity instanceof EntityLivingBase) {
                        EntityLivingBase living = (EntityLivingBase) entity;
                        chatBox.setLocation(dimId, living.posX, living.posY + living.getEyeHeight(), living.posZ);
                    } else {
    ...
    public String getItemStackDisplayName(ItemStack stack) {
        ...
        if (this.getHasEnderModem(stack)) {
            return StatCollector.translateToLocalFormatted(
                ...
                new Object[] {
                    StatCollector.translateToLocal("upgrade.computercraft:advanced_wireless_modem.adjective") });
    ...
    private void setComputerID(ItemStack stack, int computerID) {
        ...
        stack.getTagCompound()
            .setInteger("computerID", computerID);
    }
    ...
    private void setInstanceID(ItemStack stack, int instanceID) {
        ...
        stack.getTagCompound()
            .setInteger("instanceID", instanceID);
    }
