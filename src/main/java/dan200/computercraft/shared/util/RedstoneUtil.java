...
public class RedstoneUtil {
   ...
   public static void propogateRedstoneOutput(World world, int x, int y, int z, int side) {
      ...
      if (neighbour != null) {
         world.func_147460_e(neighbourX, neighbourY, neighbourZ, block);
         if (neighbour.isNormalCube(world, neighbourX, neighbourY, neighbourZ)) {
            world.notifyBlocksOfNeighborChange(neighbourX, neighbourY, neighbourZ, neighbour, Facing.oppositeSide[side]);
         }
      }
   }
}
