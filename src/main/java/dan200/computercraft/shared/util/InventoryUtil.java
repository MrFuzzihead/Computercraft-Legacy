...
public class InventoryUtil {
    ...
    public static boolean areItemsStackable(ItemStack a, ItemStack b) {
        return a == b ? true
            : a != null && b != null
                ...
                && (a.stackTagCompound == null && b.stackTagCompound == null
                    || a.stackTagCompound != null && b.stackTagCompound != null
    ...
    public static IInventory getInventory(World world, int x, int y, int z, int side) {
        ...
        Vec3 vecStart = Vec3.createVectorHelper(
            x + 0.5 + 0.6 * Facing.offsetsXForSide[side],
            y + 0.5 + 0.6 * Facing.offsetsYForSide[side],
            z + 0.5 + 0.6 * Facing.offsetsZForSide[side]);
        Vec3 vecDir = Vec3
            .createVectorHelper(Facing.offsetsXForSide[dir], Facing.offsetsYForSide[dir], Facing.offsetsZForSide[dir]);
                ...
                for (int n = 0; n < slots.length; n++) {
                    ...
                    if (canPlaceItemThroughFace(inventory, slot, remainder, face)) {
                        ...
                        if (slotContents == null) {
                            ...
                            int space = Math.min(slotContents.getMaxStackSize(), inventory.getInventoryStackLimit())
            ...
            for (int n = 0; n < slots.length; n++) {
                ...
                if (countRemaining > 0) {
                    ...
                    if (stack != null && canTakeItemThroughFace(inventory, slot, stack, face)
                        && (partialStack == null || areItemsStackable(stack, partialStack))) {
                        if (countRemaining >= stack.stackSize) {
                            inventory.setInventorySlotContents(slot, null);
                            if (partialStack == null) {
                                partialStack = stack;
                                countRemaining = Math.min(
                                    countRemaining,
                                    stack.getItem()
