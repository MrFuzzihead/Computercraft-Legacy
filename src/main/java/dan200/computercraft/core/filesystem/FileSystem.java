...
public class FileSystem {
   ...
   public void unload() {
      synchronized (this.m_openFiles) {
         while (this.m_openFiles.size() > 0) {
            IMountedFile file = this.m_openFiles.iterator().next();
   ...
   private synchronized void copyRecursive(
      String sourcePath, FileSystem.MountWrapper sourceMount, String destinationPath, FileSystem.MountWrapper destinationMount
   ...
   private FileSystem.MountWrapper getMount(String path) throws FileSystemException {
      Iterator<FileSystem.MountWrapper> it = this.m_mounts.values().iterator();
