...
public class TurtleCompareCommand implements ITurtleCommand {
   ...
   public TurtleCommandResult execute(ITurtleAccess turtle) {
      ...
      if (WorldUtil.isBlockInWorld(world, newPosition) && !world.isAirBlock(newPosition.posX, newPosition.posY, newPosition.posZ)
         )
       {
         Block lookAtBlock = world.getBlock(newPosition.posX, newPosition.posY, newPosition.posZ);
         if (lookAtBlock != null && !lookAtBlock.isAir(world, newPosition.posX, newPosition.posY, newPosition.posZ)) {
            ...
            if (!lookAtBlock.hasTileEntity(lookAtMetadata)) {
               try {
                  Method method = ReflectionHelper.findMethod(
                     Block.class, lookAtBlock, new String[]{"func_149644_j", "j", "createStackedBlock"}, new Class[]{int.class}
                  );
                  if (method != null) {
            ...
            for (int i = 0; i < 5 && lookAtStack == null; i++) {
               ArrayList<ItemStack> drops = lookAtBlock.getDrops(
                  world, newPosition.posX, newPosition.posY, newPosition.posZ, lookAtMetadata, 0
               );
