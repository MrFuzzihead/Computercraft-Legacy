...
public class TurtleMoveCommand implements ITurtleCommand {
   ...
   public TurtleCommandResult execute(ITurtleAccess turtle) {
      ...
      if (!canEnterResult.isSuccess()) {
         ...
         if (block != null
            && !oldWorld.isAirBlock(newPosition.posX, newPosition.posY, newPosition.posZ)
            ...
            if (!oldWorld.checkNoEntityCollision(aabb)) {
               ...
               for (int i = 0; i < list.size(); i++) {
                  ...
                  if (!entity.isDead && entity.preventEntitySpawning && entity.getBoundingBox() != null) {
                     AxisAlignedBB pushedBB = entity.getBoundingBox()
                        .offset(Facing.offsetsXForSide[direction], Facing.offsetsYForSide[direction], Facing.offsetsZForSide[direction]);
                     if (!oldWorld.func_147461_a(pushedBB).isEmpty()) {
                        return TurtleCommandResult.failure("Movement obstructed");
                     }
                  }
               }
