...
public class TurtleBrain implements ITurtleAccess {
   ...
   public static void cleanupBrains() {
      if (s_allClientBrains.size() > 0) {
         Iterator<Entry<Integer, WeakReference<TurtleBrain>>> it = s_allClientBrains.entrySet().iterator();
   ...
   public TurtleBrain getFutureSelf() {
      if (this.getOwner().getWorldObj().isRemote) {
   ...
   public void writeToNBT(NBTTagCompound nbttagcompound) {
      ...
      if (this.m_upgradeNBTData.containsKey(TurtleSide.Left)) {
         nbttagcompound.setTag("leftUpgradeNBT", (NBTTagCompound)this.getUpgradeNBTData(TurtleSide.Left).copy());
      }

      if (this.m_upgradeNBTData.containsKey(TurtleSide.Right)) {
         nbttagcompound.setTag("rightUpgradeNBT", (NBTTagCompound)this.getUpgradeNBTData(TurtleSide.Right).copy());
      }
   }
   ...
   public Object[] executeCommand(ILuaContext context, ITurtleCommand command) throws LuaException, InterruptedException {
      if (this.getWorld().isRemote) {
         ...
         } while (response.length < 3 || !(response[1] instanceof Number) || !(response[2] instanceof Boolean) || ((Number)response[1]).intValue() != commandID);
   ...
   public Vec3 getRenderOffset(float f) {
      switch (this.m_animation) {
         ...
         case MoveDown:
            ...
            return Vec3.createVectorHelper(distance * Facing.offsetsXForSide[dir], distance * Facing.offsetsYForSide[dir], distance * Facing.offsetsZForSide[dir]);
         default:
   ...
   public float getToolRenderAngle(TurtleSide side, float f) {
      return (side != TurtleSide.Left || this.m_animation != TurtleAnimation.SwingLeftTool)
            && (side != TurtleSide.Right || this.m_animation != TurtleAnimation.SwingRightTool)
         ? 0.0F
      ...
      if (serverComputer != null) {
         for (TurtleSide side : TurtleSide.values()) {
            ...
            if (peripheral != null) {
               ...
               } else if (!this.m_peripherals.get(side).equals(peripheral)) {
                  serverComputer.setPeripheral(dir, peripheral);
                  serverComputer.setRedstoneInput(dir, 0);
                  serverComputer.setBundledRedstoneInput(dir, 0);
   ...
   private void updateAnimation() {
      if (this.m_animation != TurtleAnimation.None) {
         ...
         if (ComputerCraft.turtlesCanPush
            && (
               this.m_animation == TurtleAnimation.MoveForward
                  || this.m_animation == TurtleAnimation.MoveBack
            ...
            AxisAlignedBB aabb = AxisAlignedBB.getBoundingBox(
               pos.posX, pos.posY, pos.posZ, pos.posX + 1.0, pos.posY + 1.0, pos.posZ + 1.0
            );
