...
public class TurtlePlaceCommand implements ITurtleCommand {
      ...
      if (stack == null) {
         ...
         if (remainder != stack) {
            ...
            return stack.getItem() instanceof ItemBlock
               ? TurtleCommandResult.failure("Cannot place block here")
               : TurtleCommandResult.failure("Cannot place item here");
         }
      }
   }

   public static ItemStack deploy(ItemStack stack, ITurtleAccess turtle, int direction, Object[] extraArguments, String[] o_errorMessage) {
      ...
      if (remainder != stack) {
         ...
         remainder = deployOnBlock(stack, turtle, turtlePlayer, newPosition, Facing.oppositeSide[direction], extraArguments, true, o_errorMessage);
         if (remainder != stack) {
            return remainder;
         } else {
            ...
            if (remainder != stack) {
               ...
               if (direction >= 2) {
                  remainder = deployOnBlock(stack, turtle, turtlePlayer, WorldUtil.moveCoords(newPosition, 0), 1, extraArguments, false, o_errorMessage);
                  if (remainder != stack) {
                     return remainder;
                  }
               }
   ...
   public static void orientPlayer(ITurtleAccess turtle, TurtlePlayer turtlePlayer, ChunkCoordinates position, int direction) {
   ...
   private static ItemStack deployOnEntity(
      ItemStack stack, final ITurtleAccess turtle, TurtlePlayer turtlePlayer, int direction, Object[] extraArguments, String[] o_errorMessage
   ) {
   ...
   private static boolean canDeployOnBlock(
      ItemStack stack, ITurtleAccess turtle, TurtlePlayer player, ChunkCoordinates position, int side, boolean allowReplaceable, String[] o_errorMessage
   ) {
      World world = turtle.getWorld();
      if (WorldUtil.isBlockInWorld(world, position)
         && !world.isAirBlock(position.posX, position.posY, position.posZ)
         ...
         if (allowReplaceable || !replaceable) {
            if (ComputerCraft.turtlesObeyBlockProtection) {
               ...
               if (replaceable) {
                  editable = ComputerCraft.isBlockEditable(world, position.posX, position.posY, position.posZ, player);
               } else {
                  ChunkCoordinates shiftedPos = WorldUtil.moveCoords(position, side);
                  if (WorldUtil.isBlockInWorld(world, shiftedPos)) {
                     editable = ComputerCraft.isBlockEditable(world, shiftedPos.posX, shiftedPos.posY, shiftedPos.posZ, player);
                  }
               }
   ...
   private static ItemStack deployOnBlock(
      ItemStack stack,
      ...
      if (!canDeployOnBlock(stack, turtle, turtlePlayer, position, side, allowReplace, o_errorMessage)) {
         ...
         if (item.onItemUseFirst(
               stackCopy, turtlePlayer, turtle.getWorld(), position.posX, position.posY, position.posZ, side, hitX, hitY, hitZ
            )
            || item.onItemUse(
               stackCopy, turtlePlayer, turtle.getWorld(), position.posX, position.posY, position.posZ, side, hitX, hitY, hitZ
            )) {
