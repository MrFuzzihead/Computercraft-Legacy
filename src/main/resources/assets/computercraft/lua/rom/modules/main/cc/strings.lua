...
local function wrap(text, width)
    ...
    local function push_line()
        lines_n = lines_n + 1
        lines[lines_n] = current_line:gsub("%s+$", "")
        current_line = ""
    end
...
--[[- Split a string into parts, each separated by a delimiter.

For instance, splitting the string `"a b c"` with the delimiter `" "` returns
a table with three strings: `"a"`, `"b"`, and `"c"`.

By default the delimiter is treated as a Lua pattern. Pass `true` as the
