...
Ported from CC:Tweaked. `debug.getinfo` / `debug.getlocal` are not available
in this build; function display falls back to plain `tostring(fn)` (the
upstream code already guards both calls, so no behaviour change).
...
local type, getmetatable, setmetatable, colours, str_write, tostring =
    type, getmetatable, setmetatable, colours, write, tostring

-- debug is not loaded in this build; the show_function guards handle nil safely.
local debug_info, debug_local = debug and debug.getinfo, debug and debug.getlocal
...
local function concat(...)
    -- Use select('#', ...) rather than table.pack so that we are not reliant on
...
-- Returns how many characters remain on the line after rendering doc, given
-- `width` characters are currently available.  A negative result means the
-- content would overflow.
-- Note: `group` and `nest` nodes only appear in the *unflattened* half of a
-- group pair; get_remaining is only called on the *flattened* half (doc[1]),
-- so these branches are unreachable in practice — but are kept for safety.
local function get_remaining(doc, width)
    ...
    elseif kind == "group" or kind == "nest" then
        return get_remaining(doc[1], width) -- upstream had kind[1] (typo); fixed here
    else
        error("Unknown doc " .. kind)
    end
end
...
--
-- @tparam      Doc    doc         The document to render.
-- @tparam[opt] number width       The maximum width of this document.
-- @tparam[opt=0.6] number ribbon_frac The maximum fraction of the width that we should write in.
...
local function show_function(fn, options)
    -- debug_info is nil when the debug library is not loaded; fall back to tostring.
...
--    `function: 0xXXXXXXXX` (`false` by default).
