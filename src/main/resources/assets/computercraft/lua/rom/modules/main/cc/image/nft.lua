...
--
-- Ported from CC:Tweaked. The upstream `load` function uses `io.open`; this
-- port uses `fs.open` instead, which is the standard file API in this
-- environment and correctly returns a second error-string value on failure.
...
-- loaded.
--
-- Note: upstream CC:Tweaked uses `io.open`; this port uses `fs.open` so that
-- a proper error string is returned when the file cannot be opened.
