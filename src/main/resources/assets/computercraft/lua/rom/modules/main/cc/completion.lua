...
-- @since 1.85.0
--
-- Ported from CC:Tweaked. `peripheral.getNames()` does not exist in this
-- build; `completion.peripheral` instead checks each redstone side directly.

local expect = require "cc.expect".expect

-- Upstream uses peripheral.getNames() which is not available here.
-- Fall back to checking each side reported by redstone.getSides().
local function get_peripheral_names()
    local names = {}
...
local function peripheral_(text, add_space)
    ...
    return choice_impl(text, get_peripheral_names(), add_space)
end
...
--
-- @tparam string text The input string to complete.
-- @tparam[opt] boolean add_space Whether to add a space after the completed setting.
